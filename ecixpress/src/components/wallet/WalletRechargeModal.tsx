...
import { PAYMENT_METHODS, getMethodMeta } from "./paymentMethods";
import {
  createTopup,
  getPseInstitutions,
  getTopupDetails,
...
const WalletRechargeModal: React.FC<Props> = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open && method === "PSE" && banks.length === 0 && userId) {
      getPseInstitutions(userId)
        .then(setBanks)
        .catch(() => {
          /* opcional; el usuario puede reintentar */
        });
    }
  }, [open, method, banks.length, userId]);

  const fieldsError = useMemo(() => {
    if (amountCents < MIN_PESOS * 100)
      return `El mínimo es ${formatCOP(MIN_PESOS * 100)}.`;
    ...
    if (meta.fields.includes("card")) {
      ...
      if (!/^\d{2}$/.test(cardExpMonth))
        return "Mes de vencimiento inválido (MM).";
      if (!/^\d{2}$/.test(cardExpYear))
        return "Año de vencimiento inválido (AA).";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "No se pudo iniciar la recarga",
      );
    } finally {
      setSubmitting(false);
    }
      ...
      {meta.fields.includes("pse") && (
        <>
          ...
          <div>
            ...
            >
              <option value="">
                {banks.length ? "Selecciona tu banco" : "Cargando bancos…"}
              </option>
              {banks.map((b) => (
                <option
          ...
          <div>
            ...
            <input
              ...
              placeholder={sd.customerName ?? "Juan Pérez"}
              className={input}
            />
          </div>
          <div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              placeholder={sd.cardHolder ?? "JUAN PEREZ"}
              className={input}
            />
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div>
              ...
              <input
                ...
                placeholder={sd.cardExpMonth ?? "08"}
                className={input}
              />
            </div>
            <div>
  ...
  const renderResult = () => {
    ...
    const createExtra = (
      result?.wompi?.payment_method as Record<string, any> | undefined
    )?.extra;
    const qrImage =
      details?.qrImage ?? (createExtra?.qr_image as string | undefined) ?? null;
    const approveUrl =
      details?.redirectUrl ??
      result?.redirectUrl ??
    ...
    const approved =
      liveStatus === "APPROVED" || details?.status === "APPROVED";
    const declined =
      liveStatus === "DECLINED" ||
      liveStatus === "ERROR" ||
      liveStatus === "VOIDED" ||
      ...
      <div className="space-y-5 text-center">
            ...
            {qrImage ? (
              <div className="flex flex-col items-center gap-2">
                <p className="text-sm text-gray-600">
                  Escanea este código con tu app Bancolombia:
                </p>
                <img
              ...
              <p className="text-sm text-gray-600">
                Te enviamos una notificación a tu app <strong>Nequi</strong>.
                Aprueba el pago desde el celular para acreditar el saldo.
              </p>
            ) : (
              <p className="text-sm text-gray-500">
                {checking
                  ? "Obteniendo datos del pago…"
                  : "Aún no hay un enlace de aprobación disponible. Intenta actualizar el estado."}
              </p>
            )}

            <p className="text-xs text-gray-400">
              El saldo se acredita cuando la pasarela confirma el pago. Tras
              aprobar, pulsa «Verificar estado».
            </p>
          </>
        )}
        ...
        {declined && (
          <p className="text-sm text-red-600">
            La transacción fue rechazada o cancelada. Puedes intentar una nueva
            recarga.
          </p>
        )}
  ...
  const titles: Record<Step, { t: string; s?: string }> = {
    form: {
      t: "Recargar billetera",
      s: "Agrega saldo a tu billetera ECIExpress",
    },
        ...
        <div className="mt-1 flex justify-center">
          <div className="relative">
            ...
            <input
              ...
              value={amountDisplay}
              onChange={(e) =>
                setAmountPesos(e.target.value.replace(/\D/g, ""))
              }
              placeholder="0"
              size={1}
              style={{ width: `${Math.max(amountDisplay.length || 1, 1)}ch` }}
              className="bg-transparent text-center text-4xl font-extrabold text-gray-900 placeholder-gray-300 focus:outline-none"
            />
          </div>
        </div>
