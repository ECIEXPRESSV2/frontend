  ...
  return (
      <section className="relative py-20 md:py-28 px-6 overflow-hidden bg-gradient-to-b from-yellow-50 via-white to-cyan-50">
        ...
        <div className="relative max-w-4xl mx-auto text-center space-y-10">
          ...
          <div className="pt-6 rounded-2xl bg-white border border-gray-100 shadow-sm grid grid-cols-3 divide-x divide-gray-100 overflow-hidden">
            ...
            {[
              ...
              { value: '< 2 min', label: 'Tiempo promedio de recogida' },
                ...
                >
                  ...
                  <p className="font-body text-xs md:text-sm text-gray-500 mt-1">{item.label}</p>
                </div>
            ))}
...
const FOOTER_LINK_CLASS = 'hover:text-white transition hover:translate-x-1 inline-block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded';
const SOCIAL_LINK_CLASS = 'text-gray-400 hover:text-white transition hover:scale-110 transform duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2';
...
const FOOTER_COLUMNS: FooterColumn[] = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Contacto', href: 'mailto:support@ecixpress.com' },
    ],
  },
];
...
const SOCIAL_LINKS = [
  ...
  {
    label: 'GitHub',
    path: 'M23.953 4.57a10 10 0 002.856-3.51 10 10 0 01-2.856.175 10 10 0 00-7.737-4.247 9.753 9.753 0 00-7.738 4.247 10 10 0 002.856-.175 10 10 0 00.637 2.482c-4.547 2.177-7.843 6.782-7.843 12.018 0 1.315.236 2.59.697 3.832A9.975 9.975 0 002 24h2.969a5.99 5.99 0 005.924-4.487 5.987 5.987 0 005.924 4.487h2.969c1.238-3.226 2.048-6.797 2.048-10.57 0-5.236-3.296-9.841-7.843-12.018z',
  },
  {
    label: 'Twitter',
    path: 'M19.633 7.997c.013.175.013.349.013.523 0 5.325-4.053 11.461-11.46 11.461-2.282 0-4.402-.661-6.186-1.809.324.037.636.05.973.05 1.883 0 3.616-.605 5.038-1.637-1.881-.037-3.463-1.27-4.066-2.964.623.095 1.209.06 1.765-.196-1.963-.395-3.431-2.128-3.431-4.202v-.051c.575.325 1.231.518 1.931.542-1.15-.766-1.912-2.081-1.912-3.569 0-.785.212-1.51.582-2.14 2.12 2.589 5.298 4.29 8.889 4.467-.072-.326-.112-.658-.112-.991 0-2.396 1.944-4.338 4.338-4.338 1.249 0 2.374.533 3.154 1.385 1.003-.196 1.946-.561 2.795-1.061-.328 1.016-.999 1.869-1.88 2.41.896-.089 1.772-.346 2.577-.893-.593.875-1.346 1.641-2.21 2.26z',
  },
];

const FooterColumnList: React.FC<{ column: FooterColumn }> = ({ column }) => (
  <div className="space-y-4">
    <h4 className="font-semibold">{column.title}</h4>
    <ul className="space-y-2 text-sm text-gray-400">
      {column.links.map((link) => (
        <li key={link.label}>
          <a href={link.href} className={FOOTER_LINK_CLASS}>{link.label}</a>
        </li>
      ))}
    </ul>
  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4 pt-2">
              {SOCIAL_LINKS.map((social) => (
                <a key={social.label} href="#" className={SOCIAL_LINK_CLASS} aria-label={social.label}>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              ...
              <li className="flex items-start gap-2 group">
                ...
                <span>Escuela Colombiana de Ingeniería Julio Garavito, Bogotá D.C.</span>
              </li>
            </ul>
          </div>
        </div>
