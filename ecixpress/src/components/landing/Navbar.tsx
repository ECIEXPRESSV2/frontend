...
const Navbar: React.FC<NavbarProps> = ({ onLoginClick, onSignUpClick }) => {
  ...
  return (
    // El transform/will-change vive en el div interno, no en el `nav` con `sticky`:
    // en iOS Safari un `transform` en el propio elemento sticky rompe el "pegado"
    // y el navbar queda flotando/saltando al hacer scroll.
    <nav className="sticky top-3 z-50 w-full mx-3 sm:mx-6 mt-4">
      <div className="w-full bg-gradient-to-r from-white/80 to-yellow-50/60 backdrop-blur-xl border border-white/40 shadow-lg rounded-xl [transform:translateZ(0)] [-webkit-transform:translateZ(0)] [backface-visibility:hidden] will-change-transform">
        <div className="w-full px-6 md:px-12 py-3 md:py-4">
